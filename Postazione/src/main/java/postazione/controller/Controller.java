...
public class Controller extends TerminalController {
	 ...
	 * @param post Classe model.
	 */
	public Controller(ViewInterface view, Post post, PostaDB db) throws PEException {
		super(new Server(new Factory(), Constants.portPost, post.getNumConnections(), Terminals.Type.Post), view,
		...
		if (Constants.linkSSL) {
			...
			try {
				System.setProperty("javax.net.ssl.keyStore", "ssl/keystore_bis.jks");
				System.setProperty("javax.net.ssl.keyStorePassword", CfgManager.getPassword("ks_bis"));
				System.setProperty("javax.net.ssl.trustStore", "ssl/truststore_bis.jks");
				System.setProperty("javax.net.ssl.trustStorePassword", CfgManager.getPassword("ts_bis"));
			} catch (PEException e) {
	...
	public void activate(String sessionKey) {
		...
		logger.debug("activate {} ", sessionKey);
	...
	public boolean selectCandidate(int ballotNum, String listName, String candidateID, boolean add) {
		// Si verifica se il candidato sta realmente presentandosi per la lista nella
		// scheda i-esima
	...
	/**
	 * Funzione richiamata quando il server riceve una richiesta di associazione da
	 * parte del seggio.
	 * 
	...
	Message setAssociation(int[] ballotCodes, String badge) throws PEException {
		// Se la postazione non è attiva informiamo il seggio che non possiamo
		// associarla
	...
	/**
	 * Funzione richiamata quando il seggio legge il badge associato a questa
	 ...
	 *                     registrato per qualsiasi motivo.
	 */
	...
	/**
	 * Funzione che notifica il seggio di un cambiamento di stato. Viene richiamata
	 * sia da funzioni server che client, dato che i cambiamenti di stato possono
	 * essere causati sia dalla view che dal seggio.
	 * 
	 * @param ipStation L'ip del mittente, utile nel caso la postazione non sia
	 *                  stata ancora attivata e quindi non conosca l'ip del proprio
	 *                  seggio.
	 */
	void notifyStateToStation(InetAddress ipStation) throws PEException {
		logger.debug("notifyStateToStation {} ", ipStation);
		// Questo metodo viene chiamato con null da badgeIn(), con l'IP di chi ha
		// avviato la connessione da Service
		if (ipStation == null) {
	...
	/**
	 * Funzione il cui scopo è gestire quasi qualunque tipo di errore di
	...
	/**
	 * Funzione che permette di usare la postazione come una macchina a stati
	...
	/**
	 * Funzione richiamata quando un badge è letto dal reader RFID. Verifica se il
	...
	/**
	 * Funzione che aggiorna lo stato da associata a "in uso" se il badge presentato
	 * è quello corretto.
	 * 
...
//			return false;
	...
	private boolean esBadgeMail(String newBadge) {
		// esBadgeMail
		// 1.- Comprueba si es un badge de un votante con email
		if (postaDB.esBadgeMail(newBadge)) {
			post.setState(StatePost.IN_USO);
			updateView();
			return true;
		...
		// 2.- si lo es
		// 2.1.- fija estado post.setState(IN_USO);
		// 2.2.- updateView()
		// 2.3.- RETURN TRUE
		// 3.- si no
		// 3.1- RETURN FALSE
	...
	/**
	 * Funzione che contatta l'urna per ottenere gli nonce necessari per inviare i
	 * voti.
	 * 
	...
	private void sendVotes() throws PEException {
		// Recuperiamo chiave pubblica, chiave di sessione e (una copia delle) schede
		// compilate
	...
	/**
	 * Verifica che l'ip dal quale è arrivato un messaggio il cui mittente dovrebbe

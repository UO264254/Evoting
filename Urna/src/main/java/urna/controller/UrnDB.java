...
public class UrnDB extends DB {
	...
	public ArrayList<Session> getSessions(String username) throws PEException {
		String query = "SELECT P.Code, P.Name, P.Supervisor, S.Code, S.StartsAt, S.EndsAt, (S.StartsAt <= NOW() AND S.EndsAt >= NOW()) AS Validity "
				...
				+ "WHERE P.Starts <= NOW() AND P.Ends >= NOW() AND P.Supervisor = ? " + "ORDER BY P.Code, S.Code ;";
		...
		try (ConnectionManager cm = dbms.getConnectionManager()) {
			...
			while (rs.next()) {
				...
				LocalDateTime start = rs.getTimestamp("S.StartsAt").toLocalDateTime(),
						end = rs.getTimestamp("S.EndsAt").toLocalDateTime();
	...
	public EmptyBallot[] getEmptyBallots(int procedureCode) throws PEException { // rivedere e semplificare?
		...
		/*
		 * String candidatesQuery = "SELECT * " + "FROM Candidate1 AS C " +
		 * "JOIN Running AS R ON C.ID = R.CandidateID " +
		 * "JOIN Ballot AS B ON (R.BallotCode = B.Code AND R.ProcedureCode = B.ProcedureCode) "
		 * + "LEFT JOIN  " + "	( " + "    SELECT * " + "    FROM ElectoralList AS EL "
		 * + "    JOIN Member AS M ON EL.Code = M.ElectoralListCode " +
		 * "    WHERE M.ProcedureCode = ? " + "	) AS PM ON C.ID = PM.CandidateID " +
		 * "WHERE B.ProcedureCode = ? " + "ORDER BY B.Code ;";
		 */

		String candidatesQuery = "SELECT * " + "FROM Candidate AS C  "
				+ "JOIN Running AS R ON C.ProcedureCode = R.ProcedureCode AND C.ID = R.CandidateID  "
				+ "JOIN Ballot AS B ON R.ProcedureCode = B.ProcedureCode AND R.BallotCode = B.Code " + "LEFT JOIN   "
				+ "(  " + "SELECT M.*, EL.Name  " + "FROM ElectoralList AS EL  " + "JOIN Member AS M "
		...
		String optionsQuery = "SELECT B.*, RO.* " + "FROM ReferendumOption AS RO "
		...
		try (ConnectionManager cManager = dbms.getConnectionManager()) {
			...
			while (rs.next()) {
				...
				if (!ballotMap.containsKey(ballotCode)) {
					ballotMap.put(ballotCode, new EmptyBallot(rs.getString("B.Name"), rs.getInt("B.Code"),
							rs.getString("B.Description"), rs.getInt("B.MaxPreferences")));
				}
				...
				if (eL == null) {
					...
					if (listName == null) {
						listName = FileUtils.getResourceBundle("etiqueta_candidatos_no_lista");
					}
				...
				eL.addPerson(new Person(rs.getString("C.FirstName"), rs.getString("C.LastName"), dateOfBirth,
						rs.getString("C.ID")));
			}
			...
			while (rs.next()) {
				...
				if (eB == null) {
					eB = new EmptyBallot(rs.getString("B.Name"), rs.getInt("B.Code"), rs.getString("B.Description"),
							rs.getInt("B.MaxPreferences"));
	...
	public String getTerminalSessionKey(int procedureCode, int sessionCode, InetAddress ipTerminal, Terminals.Type type)
			throws PEException {
		/*
		 * String query = "SELECT SymmetricKey " + "FROM Terminal AS T " +
		 * "JOIN SessionKey as SK on T.ID = SK.TerminalID " + "WHERE T.IPAddress = ? " +
		 * "AND Type = ? " + "AND SK.ProcedureCode = ? " + "AND SK.SessionCode = ? ;";
		 */

		String query = "SELECT T.SymmetricKey " + "FROM Terminal AS T " + "JOIN SessionKey AS SK "
				+ "ON T.ProcedureCode = SK.ProcedureCode " + "AND T.SessionCode = SK.SessionCode "
				+ "AND T.ID = SK.TerminalID " + "WHERE T.IPAddress = ? " + "AND T.Type = ? "
				+ "AND SK.ProcedureCode = ? " + "AND SK.SessionCode = ? ;";

		try (ConnectionManager cManager = dbms.getConnectionManager()) {
			ResultSet rs = cManager.executeQuery(query, ipTerminal.getHostAddress(), type.toString(), procedureCode,
					sessionCode);
	...
	public String getStationIP(int procedureCode, int sessionCode, InetAddress ipTerminal, boolean isPost)
			throws PEException {
		/*
		 * String query = "SELECT T.ID, S.IPAddress as IP " + "FROM Terminal as S " +
		 * "LEFT JOIN IsStationOf as ISO on S.ID = ISO.Station " +
		 * "LEFT JOIN Terminal as T on ISO.Terminal = T.ID " +
		 * "WHERE S.Type = 'Station' " + "AND T.Type = ? " + "AND T.IPAddress = ? ;";
		 */

		String query = "SELECT T.ID, S.IPAddress as IP " + "FROM Terminal AS T " + "LEFT JOIN IsStationOf AS ISO "
				+ "ON T.ProcedureCode = ISO.ProcedureCode " + "AND T.SessionCode = ISO.SessionCode "
				+ "AND T.ID = ISO.Terminal " + "LEFT JOIN Terminal AS S " + "ON ISO.ProcedureCode = S.ProcedureCode "
				+ "AND ISO.SessionCode = S.SessionCode " + "AND ISO.Station = S.ID " + "WHERE S.Type = 'Station' "
				+ "AND T.ProcedureCode = ? " + "AND T.SessionCode = ? " + "AND T.Type = ? " + "AND T.IPAddress = ? ;";

		try (ConnectionManager connManager = dbms.getConnectionManager()) {
			ResultSet rs = connManager.executeQuery(query, procedureCode, sessionCode, isPost ? "Post" : "SubStation",
					ipTerminal.getHostAddress());
	...
	public void getTerminalsIPs(int procedureCode, int sessionCode, InetAddress ipStation,
			ArrayList<InetAddress> ipPosts, ArrayList<InetAddress> ipSubStations) throws PEException {
		/*
		 * String query = "SELECT T.ID, T.IPAddress AS IP " + "FROM Terminal AS S " +
		 * "LEFT JOIN IsStationOf AS ISO ON S.ID = ISO.Station " +
		 * "LEFT JOIN Terminal AS T ON ISO.Terminal = T.ID " +
		 * "WHERE S.Type = 'Station' " + "AND T.Type = ? " + "AND S.IPAddress = ? " +
		 * "ORDER BY T.ID ;";
		 */

		String query = "SELECT T.ID, T.IPAddress AS IP " + "FROM Terminal AS S " + "LEFT JOIN IsStationOf AS ISO "
				+ "ON S.ProcedureCode = ISO.ProcedureCode " + "AND S.SessionCode = ISO.SessionCode "
				+ "AND S.ID = ISO.Station " + "LEFT JOIN Terminal AS T " + "ON ISO.ProcedureCode = T.ProcedureCode "
				+ "AND ISO.SessionCode = T.SessionCode " + "AND ISO.Terminal = T.ID " + "WHERE S.ProcedureCode = ? "
				+ "AND S.SessionCode = ? " + "AND S.Type = 'Station' " + "AND S.IPAddress = ? " + "AND T.Type = ? "
				+ "ORDER BY T.ID ;";

		String ipString = null;
		try (ConnectionManager connManager = dbms.getConnectionManager()) {
			ResultSet rs = connManager.executeQuery(query, procedureCode, sessionCode, ipStation.getHostAddress(),
					"Post");
			while (rs.next()) {
				// Salviamo quì la stringa ricevuta dal DB, così da averla disponibile per il
				// catch se non dovesse essere un ip valido
	...
	public ArrayList<Person> searchPerson(int procedureCode, String similarFirstName, String similarLastName,
			int maxResults) throws PEException {
		String voterQuery = "SELECT V.*, " + "	EXISTS (" + "	SELECT * " + "	FROM HasVoted AS HV "
				+ "	WHERE ProcedureCode = V.ProcedureCode " + "	AND VoterID = V.ID) AS AlreadyVoted "
				+ "FROM Voter AS V " + "WHERE FirstName like ? AND LastName like ? " + "AND ProcedureCode = ? "
				+ "LIMIT ? ;";

		String ballotsListQuery = "SELECT * " + "FROM VoterBallotsList AS VBL " + "WHERE VBL.VoterID = ? "
				+ "AND VBL.ProcedureCode = ? ;";

		ArrayList<Person> voters = new ArrayList<>();
		try (ConnectionManager cManager = dbms.getConnectionManager()) {
			ResultSet rs = cManager.executeQuery(voterQuery, "%" + similarFirstName + "%", "%" + similarLastName + "%",
					procedureCode, maxResults + 1);
			...
			while (rs.next()) {
				...
				voters.add(new Person(rs.getString("V.FirstName"), rs.getString("V.LastName"), rs.getString("V.Email"),
						voterID, ballotCodes, !rs.getBoolean("AlreadyVoted"), rs.getString("V.DateOfBirth")));
			}
		} catch (SQLException e) {
			throw DBException.DB_0(e);
		}
	...
	public boolean esBadgeMail(String newBadge) throws PEException {
	...
	/**
	 * Funzione usata per verificare se esiste già un votante con l'id specificato,
	 * legato alla procedura attuale, in modo da poter verificare se è possibile l'
	 * aggiunzione di un nuovo votante con quell'ID.
	 * 
	...
	public Person getVoter(int procedureCode, String voterID) throws PEException {
		...
		try (ConnectionManager cManager = dbms.getConnectionManager()) {
			...
			if (rs.next())
				return new Person(rs.getString("FirstName"), rs.getString("LastName"), rs.getString("Email"), voterID,
						null, false, rs.getString("DateOfBirth"));
		} catch (SQLException e) {
			throw DBException.DB_0(e);
		}
	...
	public void registerNewVoter(int procCode, InetAddress ipStation, String id, String ln, String fn, String birthDate,
			int[] ballots) throws PEException {
		...
		try {
			...
			cManager.executeUpdate(voterUpdate, procCode, id, fn, ln,
					LocalDate.parse(birthDate, DateTimeFormatter.ofPattern("d/M/yyyy")), added);
	...
	private void enableUser(ConnectionManager cm, int procCode, InetAddress stationIp, String voterID, int[] ballots,
			boolean newVoter) throws PEException, SQLException {
		String hasVotedQuery = "SELECT * " + "FROM HasVoted " + "WHERE ProcedureCode = ? " + "AND VoterID = ? ;";
		...
		if (!newVoter) {
			String removeBallotsUpdate = "DELETE " + "FROM VoterBallotsList " + "WHERE ProcedureCode = ? "
					+ "AND VoterID = ? ;";
	...
	public void verifyVoteData(int procedureCode, int sessionCode, String voterID, WrittenBallot[] ballots,
			String ipStation, String ipPost) throws PEException {
	...
	/**
	 * Verifica se la sessione è ancora valida in risposta alla ricezione di un
	 * voto, o se il voto è arrivato al di fuori della sessione.
	 *
	 * @return La stringa contenuta in Protocol.success in caso di successo,
	 *         l'errore riscontrato altrimenti.
	 */
	private boolean verifySessionValidity(int procedureCode, int sessionCode) throws PEException {
		String query = "SELECT IF(NOW() >= S.StartsAt AND NOW() <= S.EndsAt, 1, 0) AS Valid " + "FROM `Procedure` as P "
				+ "JOIN Session as S on P.Code = S.ProcedureCode " + "WHERE P.Code = ? AND S.Code = ? ;";
	...
	/**
	 * Verifica l'esistenza del votante nel DB, la sua appartenenza all'elettorato
	 * attivo della procedura attuale, controlla che i codici delle schede (le cui
	 * preferenze sono cifrate) siano quelle che poteva votare, e controlla che non
	 * abbia già votato.
	 */
	private void verifyVoter(int procedureCode, String voterID, HashSet<Integer> ballotCodes) throws PEException {
		String voterQuery = "SELECT V.*, (HV.ProcedureCode IS NOT NULL) AS AlreadyVoted " + "FROM Voter AS V "
				+ "LEFT JOIN HasVoted AS HV ON V.ProcedureCode = HV.ProcedureCode AND V.ID = HV.VoterID "
				+ "WHERE V.ProcedureCode = ? AND V.ID =  ? ;";

		String ballotsListQuery = "SELECT * " + "FROM Voter AS V "
	...
	/**
	 * Verifique que las IP pertenezcan a 2 terminales en el DB, una en un asiento y
	 * la otra en un lugar. Comprueba también que en el DB ese puesto específico
	 * esté relacionado con el asiento específico.
	 */

	/**
	 * Verifica che gli ip appartengano a 2 terminali presenti nel DB, uno ad un
	 * seggio e l'altro ad una postazione. Controlla inoltre che nel DB quella
	 * specifica postazione sia relativa a quello specifico seggio.
	 */
	private void verifyStationAndPost(int procedureCode, int sessionCode, String ipStation, String ipPost)
			throws PEException {
		String terminalExistenceQuery = "SELECT * " + "FROM Terminal " + "WHERE IPAddress = ? " + "AND Type = ? "
				+ "AND ProcedureCode = ? " + "AND SessionCode = ? ;";

		String terminalPairingQuery = "SELECT T1.*, T2.* " + "FROM Terminal AS T1 " + "JOIN IsStationOf AS I "
				+ "ON T1.ProcedureCode = I.ProcedureCode " + "AND T1.SessionCode = I.SessionCode "
				+ "AND T1.ID = I.Station " +

				"JOIN Terminal AS T2 " + "ON I.ProcedureCode = T2.ProcedureCode "
				+ "AND I.SessionCode = T2.SessionCode " + "AND I.Terminal = T2.ID " +

				"WHERE T1.Type = 'Station' " + "AND T2.Type = 'Post' " + "AND T1.IPAddress = ? "
				+ "AND T2.IPAddress = ? " + "AND I.ProcedureCode = ? " + "AND I.SessionCode = ? ; ";

		try (ConnectionManager cManager = dbms.getConnectionManager()) {
			ResultSet rs = cManager.executeQuery(terminalExistenceQuery, ipStation, "Station", procedureCode,
					sessionCode);
	...
	public void storeVotes(int procedureCode, int sessionCode, Person voter, WrittenBallot[] ballots,
			InetAddress ipStation, InetAddress ipPost) throws PEException {
		...
		else if (!docType.equals(Person.DocumentType.CONOSCENZA_PERSONALE) && docID == null)
		...
		/*
		 * String hasVotedUpdate =
		 * "INSERT INTO HasVoted(ProcedureCode, VoterID, StationID, DocumentType, DocumentID) VALUES(?, ?, "
		 * + "	(SELECT T.ID " + "    FROM Terminal AS T " +
		 * "    WHERE T.IPAddress = ? " + "    AND T.Type = 'Station'), ?, ?) ;";
		 */

		String hasVotedUpdate = "INSERT INTO HasVoted(ProcedureCode, VoterID, StationID, DocumentType, DocumentID) "
				+ "VALUES(?, ?, " + "(SELECT T.ID " + "FROM Terminal AS T " + "WHERE T.ProcedureCode = ? "
				+ "AND T.SessionCode = ? " + "AND T.IPAddress = ? " + "AND T.Type = 'Station'), ?, ?) ;";

		String nonceDiscriminatorQuery = "SELECT MAX(NonceDiscrim) AS MaxNonceDiscrim " + "FROM Vote "
		...
		try {
			cManager.startTransaction();
			cManager.executeUpdate(hasVotedUpdate, procedureCode, voterID, procedureCode, sessionCode,
					ipStation.getHostAddress(), docType.toString(), docID);
			...
			for (WrittenBallot ballot : ballots) {
				...
				for (VotePacket packet : packets) {
					...
					ResultSet rs = cManager.executeQuery(nonceDiscriminatorQuery, procedureCode, ballotCode,
					...
					cManager.executeUpdate(voteUpdate, procedureCode, ballotCode, packet.getSolvedNonce(),
							nonceDiscriminator, packet.getEncryptedVote(), packet.getEncryptedKi(),
							packet.getEncryptedIV(), packet.getSignature());
				}
			}

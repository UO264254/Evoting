...
public class Service extends AbstrService {
	...
	protected void execute() {
		...
		case Protocol.getTokensReq:
			if (((Controller) controller).verifyIp(ip, Terminals.Type.Station, message))
				getTokens(Terminals.Type.Station);
			break;
		...
		default:
			String error = "Il terminale [IP: " + ip.getHostAddress() + "] ha inviato il seguente messaggio inatteso: "
					+ message;
	...
	private void searchPerson(Terminals.Type type) {
		...
		try {
			...
			String expectedMsg = type == Terminals.Type.Station ? Protocol.searchPersonReq
					: Protocol.searchPersonSubStationReq;
	...
	private void getTokens(Terminals.Type type) {
		if (!link.hasNextLine()) {
			return;
		}
		...
		try {
			Message request = (Message) Message.fromB64(link.read(), "seggio");
			String[] required = { "badge" };
			Class<?>[] types = { String.class };
			
			String expectedMsg = Protocol.getTokensReq;
			
			request.verifyMessage(expectedMsg, required, types, ip.getHostAddress());
			badge = request.getElement("badge");

			response = ((Controller) controller).getTokens(ip, type, badge);

...
public class Dialogs implements Callable {
	...
	public static void printError(String title, String msg, String content) {
		...
		FutureTask<String> futureTask = new FutureTask<>(new Dialogs(AlertType.ERROR, title, msg, content));
		Platform.runLater(futureTask);
	}

	public static void printException(PEException pee) {
		printError(FileUtils.getResourceBundle("etiqueta_excepcion") + " " + pee.getCode() + " "
				+ FileUtils.getResourceBundle("etiqueta_lanzada"), pee.getGeneric(), pee.getSpecific());
		pee.printStackTrace();
	}

	public static void printSuccess(String title, String msg, String content) {
		@SuppressWarnings("unchecked")
		FutureTask<String> futureTask = new FutureTask<>(new Dialogs(AlertType.INFORMATION, title, msg, content));
		Platform.runLater(futureTask);
	}

	public static void printWarning(String title, String msg, String content) {
		@SuppressWarnings("unchecked")
		FutureTask<String> futureTask = new FutureTask<>(new Dialogs(AlertType.WARNING, title, msg, content));
		Platform.runLater(futureTask);
	}

...
public class VotersParser extends CSVParser {
	...
	private static boolean verifyVoterRow(String row, ArrayList<String> errors) {
		//Il numero di campi deve essere 6
		int fieldNumber = 6;
		...
		String ID = data[0], firstName = data[1], lastName = data[2], email = data[3], ballots = data[4], dateOfBirth = data[5];
		...
		boolean validEmail = FileUtils.isEmail(email);
		...
		if(!validCode || !validFirstName || !validLastName || !validSequenceOfBallots || !validDateOfBirth) {
			errors.add("Riga: " + row +
	    			...
	    			(validEmail ? "" : "\n\t- Il email contiene caratteri non permessi") +
		...
		return newProcedure.addVoter(new Person(firstName, lastName, email, ID, ballotCodes, false, birthDate), errors);
	}

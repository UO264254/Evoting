...
/**
 * Classe controller del modulo Poll. Contiene tutte le funzioni necessarie ad
 * inizializzare il modulo e ad interagire con view e model.
 */
public class Controller extends AbstrController {
	...
	/**
	 * Inizializza il Poll col codice della procedura scelta. Verifica che il codice
	 * della procedura sia valido ed aggiorna di conseguenza model e view: porta lo
	 * stato del model a COUNTING ed aggiorna la view per permettere il caricamento
	 * della nuova scena. Altrimenti, se la verifica non va a buon fine, mostra un
	 * dialog di errore.
	 * 
	...
	public void confirmProcedure(int procCode) {
		if (procCode == -1) {
			printError(FileUtils.getResourceBundle("etiqueta_selecciona_procedimiento"),
					FileUtils.getResourceBundle("etiqueta_procedimiento_antes_confirmar"));
			return;
		}
		...
		if (proc == null) {
			printError(FileUtils.getResourceBundle("etiqueta_selecciona_procedimiento"),
					FileUtils.getResourceBundle("etiqueta_no_procedimiento_codigo"));
			return;
		}
		
		//Si el procedimiento no está terminado y está en modo Constants.devMode -> error
		if (!proc.getTerminated() && Constants.devMode) {
			printError(FileUtils.getResourceBundle("etiqueta_procedimiento_no_fin"),
					FileUtils.getResourceBundle("etiqueta_no_escrutino_procedimiento_no_finalizado"));
			return;
		}
	 ...
	 * permettere il caricamento della nuova scena.
	 * 
	 ...
	 * proprio model). Se confermato, il model viene riportato allo stato NON ATTIVO
	 * e viene aggiorna di conseguenza la view.
	 */
	public void logout() {
	...
	/**
	 * Recupera dal DB l'elenco di procedure associate all'utente loggato (tramite
	 * la funzione {@link PollDB#getProcedures(String)}, a cui passa l'username
	 * dell'utente). <br>
	 * Le procedure sono memorizzate come oggetti di tipo {@link model.Procedure
	 * Procedure}.
	 * 
	...
	/**
	 * Avvia lo spoglio dei voti (tramite
	 * {@link PollDB#countVotes(int, String, String)}, a cui passa il codice della
	 * procedura selezionata ed i dati dell'utente loggato) e restituisce i
	 * risultati elettorali. Inoltre, se almeno un pacchetto di voto ha causato
	 * errore, mostra un dialog di avvertimento.
	 * 
	 * @return Lista dei risultati elettorali, sotto forma di ArrayList di
	 *         {@link poll.view.viewmodel.BallotResult BallotResult}
	 */
	public ArrayList<BallotResult> countVotesAndGetResults() {
		try {
			...
			if (errors > 0)
				printWarning("Problemi durante lo spoglio dei voti",
						errors + " voti non sono stati conteggiati a causa di errori di cifratura/decifratura");
	...
	private void exportResultsToCSV(File file) {
		try (PrintWriter pw = new PrintWriter(new FileOutputStream(file, false))) {
			...
			for (BallotResult ballot : poll.getElectoralResults()) {
				...
				pw.println(ballotId + ";" + FileUtils.getResourceBundle("etiqueta_null") + ";"
						+ FileUtils.getResourceBundle("etiqueta_preferencias_no_expresadas") + ";"
						+ ballot.getNullPreferences());
				pw.println(ballotId + ";" + FileUtils.getResourceBundle("etiqueta_vacio") + ";"
						+ FileUtils.getResourceBundle("etiqueta_papeletas_blanco") + ";" + ballot.getEmptyBallots());
			}
			printSuccess(FileUtils.getResourceBundle("etiqueta_exportacion_completada"),
					FileUtils.getResourceBundle("etiqueta_exportacion_correcta_csv") + file.getAbsolutePath());
		} catch (IOException e) {
			printError(FileUtils.getResourceBundle("etiqueta_exportacion_fallida"),
					FileUtils.getResourceBundle("etiqueta_exportacion_incorrecta_csv"));
			e.printStackTrace();
		}
	}
		...
		try (PDDocument doc = new PDDocument()) {
			for (BallotResult ballot : poll.getElectoralResults()) {
				...
				try (PDPageContentStream currPageContents = new PDPageContentStream(doc, currPage)) {
					...
					PDImageXObject header = PDImageXObject.createFromFile("src/main/resources/img/logo_escuela.png", doc);
					...
					printPDFBallotInfo(currPageContents, FileUtils.getResourceBundle("etiqueta_id_tarjeta"),
							ballot.getId());
					printPDFBallotInfo(currPageContents, FileUtils.getResourceBundle("etiqueta_titulo"),
							ballot.getTitle());
					printPDFBallotInfo(currPageContents, FileUtils.getResourceBundle("etiqueta_descripcion"),
							ballot.getDescription());
					...
					currPageContents.newLine();
					printPDFVotes(currPageContents, FileUtils.getResourceBundle("etiqueta_null"),
							FileUtils.getResourceBundle("etiqueta_preferencias_no_expresadas_sin_puntos"),
							ballot.getNullPreferences());
					printPDFVotes(currPageContents, FileUtils.getResourceBundle("etiqueta_vacio"),
							FileUtils.getResourceBundle("etiqueta_papeletas_blanco_sin_puntos"), ballot.getEmptyBallots());
			...
			printSuccess(FileUtils.getResourceBundle("etiqueta_exportacion_completada"),
					FileUtils.getResourceBundle("etiqueta_exportacion_correcta_pdf") + file.getAbsolutePath());
		} catch (IOException e) {
			printError(FileUtils.getResourceBundle("etiqueta_exportacion_fallida"), FileUtils.getResourceBundle("etiqueta_exportacion_incorrecta_pdf"));
			e.printStackTrace();
		}
	}

	/**
	 * Stampa testo all'interno di un file PDF. Stampa il nome del campo in
	 * grassetto, dimensione 13, mentre il contenuto del campo in corsivo,
	 * dimensione 13. Utilizzata per stampare Codice, Titolo e Descrizione di ogni
	 * scheda.
	 * 
	 ...
	 */
	private void printPDFBallotInfo(PDPageContentStream contents, String fieldName, String fieldContent)
			throws IOException {
	...
	/**
	 * Stampa testo all'interno di un file PDF. Stampa i dati passati a parametro
	...
	private void printPDFVotes(PDPageContentStream contents, String code, String name, int votes) throws IOException {
		...
		line += name + ": " + votes;
		line += votes == 1 ? " voto" : " " + FileUtils.getResourceBundle("etiqueta_votos");

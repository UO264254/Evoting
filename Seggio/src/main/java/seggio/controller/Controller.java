...
public class Controller extends TerminalController {
	...
	private List<String> badgeCodeList = new ArrayList<>();
	private Badge badge;
	...
	public Controller(ViewInterface view, Station station) throws PEException {
		super(new Server(new Factory(), Constants.portStation, station.getNumConnections(), Terminals.Type.Station),
				view, Terminals.Type.Station, Constants.statRfid);
		this.station = station;
	...
	/**
	 * Función que intenta autenticarse mutuamente con la urna.
	 * 
	 * @param sessionKey La clave de sesión introducida para crear y superar las
	 *                   Desafíos basados en nonce.
	 */
	 ...
	 *                   sfide basate su nonce.
	 */
	public void activate(String sessionKey) {
		// Si el asiento ya ha sido activado, esta función no debe ser realizada
		// Se il seggio è già stato attivato allora questa funzione non deve essere
		// eseguita
		if (!correctState(StateStation.NON_ATTIVO))
			return;

		// Se comprueba que la clave de sesión tiene el formato correcto (32 caracteres
		// alfanuméricos solo en mayúsculas)
		...
		try {
			...
			if (encryptedNonce2 != null) {
				...
				// Finalmente se notifica a los puestos la activación del asiento para que éstos
				// respondan con su estado
				// Infine si notifica alle postazioni l'attivazione del seggio perché queste
				// rispondano con il proprio stato
				askStateToPosts();

				urnThread = new UrnPolling(station.getUrnIp(), this, Terminals.Type.Station);
				urnThread.start();
				logger.debug("Creación del thread PostsPolling ");
	...
	/**
	 * Prima fase della mutua autenticazione con l'urna. Se dovesse avere successo
	...
	/**
	 * Seconda fase della mutua autenticazione con l'urna. Se dovesse avere successo
	...
	private Message authenticateToUrn(String encryptedNonce2, String sessionKey) throws PEException {
		// Si tenta di risolvere la sfida dell'urna ottenendo un nonce modificato e
		// cifrato nuovamente.
	...
	/**
	 * Función utilizada para restablecer el estado de una estación activo e
	 * interrumpiendo cualquier operación que esté realizando. El principal
	 * herramienta de "resolución" de problemas para el personal del colegio
	 * electoral.
	 * 
	 * @param post El puesto a reiniciar.
	 */

	/**
	 * Funzione adoperata per resettare una postazione riportandola allo stato
	 * attivo e interrompendo qualsiasi operazione stesse compiendo. Il principale
	 * strumento di "risoluzione" problemi per lo staff del seggio.
	 * 
	...
	public void resetPost(int post) {
		...
		if (!printConfirmation(FileUtils.getResourceBundle("etiqueta_confirma_reset") + " " + ("#" + (post + 1)) + "?",
				FileUtils.getResourceBundle("etiqueta_restablecer_estado_activo")))
			return;

		// Ci connettiamo con la postazione + Inviamo la richiesta di reset e leggiamo
		// la risposta della postazione
		String response;
		...
		// Se il reset è riuscito salviamo lo stato nel seggio e stampiamo a schermo un
		...
		printError("Ricevuto Messaggio Non Riconosciuto o Inatteso",
	...
	/**
	 * Funzione che preleva le informazioni relative ad ogni postazione e le invia
	 * ai seggi ausiliari.
	 */
	...
	/**
	 * Funzione che invia all'urna il nome parziale dell'utente perchè questa
	 * recuperi e restituisca tutti i potenziali match del database.
	 * 
	 * @param searchFn Stringa simile al nome dell'utente.
	 * @param searchLn Stringa simile al cognome dell'utente.
	 * @return Un vettore contenente tutti i votanti della procedura con nome e
	 *         cognome simile.
	 */
	public Person[] retrieveVotersByName(String searchFn, String searchLn) {
		...
		if ((searchFn == null || searchFn.length() == 0) && (searchLn == null || searchLn.length() == 0)) {
			printError(FileUtils.getResourceBundle("etiqueta_imposible_busqueda"),
					FileUtils.getResourceBundle("etiqueta_especificado_criterios_busqueda"));
			return null;
		}
		...
		if (missingVoters)
			printWarning(FileUtils.getResourceBundle("etiqueta_no_se_puede_mostrar_resultados"),
					FileUtils.getResourceBundle("etiqueta_criterios_busquedad_no_estrictos"));

		if (!missingVoters && voters.length == 0)
			printError(FileUtils.getResourceBundle("etiqueta_no_se_puede_mostrar_resultados"),
					FileUtils.getResourceBundle("etiqueta_ningun_votante_se_ajusta_criterios"));

		return null;
	}

	public boolean isBadgeMail(String badge) {
		...
		// Se crea lo solicitud con el badge
		Message request = new Message(Protocol.getTokensReq);
		request.setElement("badge", badge);

		// Enviamos la solicitud y esperamos la respuesta
		Message response;
		String errMsg = FileUtils.getResourceBundle("etiqueta_imposible_contactar_urna")
				+ FileUtils.getResourceBundle("etiqueta_comprobar_urna_asiento_conectado_red");
		try (Link link = createLink(station.getUrnIp(), station.getUrnPort(), null, errMsg)) {
			if (link.isClosed()) {
				return false;
			}

			link.write(Protocol.getTokensReq);
			link.write(request.toB64());
		...
		// Recuperamos y devolvemos si el badge es válido
		boolean badgeValid = response.getElement("badgeValid");

		if (badgeValid) {
			return badgeValid;
		} else {
			printError(FileUtils.getResourceBundle("etiqueta_no_se_puede_mostrar_resultados"),
					FileUtils.getResourceBundle("etiqueta_badge_no_existente"));
		}
	...
	/**
	 * Funzione richiamata dalla view per memorizzare il votante in fase di
	 * registrazione.
	 * 
	...
	public boolean setNewVoter(VoterViewModel voterModel) {
		if (voterModel == null) {
			printError(FileUtils.getResourceBundle("etiqueta_imposible_proceder"),
					FileUtils.getResourceBundle("etiqueta_no_seleccionado_votante"));
			return false;
		}
		...
		if (!voter.mayVote()) {
			printError(FileUtils.getResourceBundle("etiqueta_votante_no_autorizado"), voter.getFirstName() + " "
					+ voter.getLastName() + " " + FileUtils.getResourceBundle("etiqueta_ya_ha_votado"));
			return false;
		}

		if (voter.getBallotCodes() == null || voter.getBallotCodes().length == 0) {
			printError("Votante Non Autorizzato", voter.getFirstName() + " " + voter.getLastName()
					+ " non risulta abilitato ad alcuna scheda della procedura.");
			return false;
		}
	...
	/**
	 * Funzione che permette la registrazione di un nuovo utente nel DB. Controlla i
	 * valori inseriti nella view (stampa eventualmente Dialog di errore se i dati
	 * sono incompleti o non corretti), crea il messaggio per l'urna e lo invia alla
	 * stessa. Ricevuta la risposta, stampa un dialog di conferma se l'operazione è
	 * andata a buon fine, altrimenti stampa un dialog d'errore.
	 * 
	 * @param id
	 * @param lastName
	 * @param firstName
	...
	public boolean registerNewUser(String id, String lastName, String firstName, String birthDate, int[] ballots) {
		if (id == null || lastName == null || firstName == null || birthDate == null) {
			printError(FileUtils.getResourceBundle("etiqueta_no_continuar_registro"),
					FileUtils.getResourceBundle("etiqueta_rellena_todos_campos"));
			return false;
		}
		...
		if (!FileUtils.isDate(birthDate)) {
			printError(FileUtils.getResourceBundle("etiqueta_no_continuar_registro"),
					FileUtils.getResourceBundle("etiqueta_formato_fecha_incorrecto"));
			return false;
		}
		...
		if (!printConfirmation(FileUtils.getResourceBundle("etiqueta_seguro_registrar_nuevo_usuario"),
				"ID: " + id + "\n" + FileUtils.getResourceBundle("etiqueta_apellido_puntos") + " " + lastName + "\n"
						+ FileUtils.getResourceBundle("etiqueta_nombre_puntos") + " " + firstName + "\n"
						+ FileUtils.getResourceBundle("etiqueta_fecha_nacimiento") + " " + birthDate + "\n"
						+ FileUtils.getResourceBundle("etiqueta_tarjetas") + " " + Arrays.toString(ballots)))
			return false;
		...
		try {
			...
			printSuccess(FileUtils.getResourceBundle("etiqueta_registracion_confirmada"),
					FileUtils.getResourceBundle("etiqueta_votante_habilitado"));
			return true;
		} catch (PEException e) {
			printError(e);
			return false;
	...
	public Person updateExistingUser(String id, int[] ballots) {
		if (id == null || id.isEmpty() || ballots.length == 0) {
			printError("Impossibile Procedere con l'Aggiornamento",
					"Assicurati di aver compilato il campo 'Matricola' e di aver scelto le schede");
			return null;
		}

		if (!printConfirmation(FileUtils.getResourceBundle("etiqueta_habilitar_voto_usuario_elegido"),
				FileUtils.getResourceBundle("etiqueta_id_votante") + " " + id + "\n"
						+ FileUtils.getResourceBundle("etiqueta_tarjetas") + " " + Arrays.toString(ballots)))
			return null;
		...
		printSuccess(FileUtils.getResourceBundle("etiqueta_actualizacion_completada"),
				FileUtils.getResourceBundle("etiqueta_el_votante") + " " + voter.getFirstName() + " "
						+ voter.getLastName() + " " + FileUtils.getResourceBundle("etiqueta_habilitado_para_votar"));

		return voter;
	}

	/*
	 * ---- Gestión del sondeo para controlar continuamente el estado de los puestos
	 * en línea ---
	 */

	/*
	 * --- Gestione del polling per controllare continuamente lo stato delle
	 * postazioni online ---
	 */
	...
	void checkForUnreachablePosts() {
		...
		logger.debug("checkForUnreachablePosts {} ", numPost);
		...
		for (int post = 0; post < numPost; post++) {
			...
			logger.debug("un post {} ", currPost.getState().toString());
			if (currPost.getState().equals(StatePost.OFFLINE))
				continue;
			logger.debug("voy a arrancar este post ");
	...
	/**
	 * Funzione per verificare che l'ip di chi sta contattando il server corrisponda
	 * a quello di una postazione o seggio ausiliario di questo seggio.
	 * 
	 * @param ip     L'ip del mittente.
	 * @param isPost Flag per indicare se il servizio richiesto vale per le
	 *               postazioni (true) o per i seggi ausiliari (false)
	 * @return Vero o falso a seconda se il mittente è una postazione o un seggio
	 *         ausiliario che ha richiesto un servizio valido per la propria
	 *         tipologia.
	 */
	boolean verifyIp(InetAddress ip, boolean isPost) {
		...
		printError("Comunicazione Inattesa", "Tentativo di connessione da parte di " + ip.toString()
				+ "(sconosciuto) come " + terminal + " di questo seggio.");
		return false;
	}

	/**
	 * Funzione che memorizza il voto cifrato inviato da una postazione in attesa di
	 * inviarlo all'urna.
	 * 
	...
	boolean storeVoteLocally(WrittenBallot[] writtenBallots, InetAddress ipPost) {
		...
		if (post == -1) {
			printError("Errore di Ricezione del Voto", "Ricevuta richiesta di invio voti dall'indirizzo:"
					+ ipPost.toString() + ", che non corrisponde ad alcuna postazione");
			return false;
		}

		// Si verifica che la postazione sia "in uso" e che sia legata ad una
		// associazione
		String errorMessage = "";
		...
		if (!acceptedStates.contains(currPostState)) {
			errorMessage += "La postazione " + post + " risulta " + currPostState
					+ " mentre dovrebbe essere ASSOCIATA, IN USO o VOTO PENDING.\n";
		}
		...
		// Si aggiorna la view e i seggi ausiliari rigurado il cambio di stato della
		// postazione
		updateViewAndSubstation();
		return true;
	}
	...
	void setStatePost(String stateAsString, InetAddress ip) {
		...
		if (post == -1) {
			printError("Aggiornamento Sconosciuto",
					"Ricevuto aggiornamento da una postazione con l'IP (sconosciuto):" + ip.toString());
			return;
		}
		...
		if (state == null) {
			printError("Aggiornamento Sconosciuto",
					"La postazione " + post + " ha inviato lo stato non riconosciuto " + stateAsString);
			return;
		}

		// Si la estación indica que está desconectada ...
		// Se la postazione comunica di essere offline ...
		if (state == StatePost.OFFLINE) {
			// ... y estaba en medio de una votación que se comunica al personal
			// ... ed era nel mezzo di una votazione lo si comunica allo staff
			List<State.StatePost> votingStates = List.of(StatePost.ASSOCIATA, StatePost.IN_USO, StatePost.VOTO_INVIATO,
					StatePost.VOTO_PENDING);
			if (votingStates.contains(station.getPostState(post)))
				printError(FileUtils.getResourceBundle("etiqueta_actualizacion_desconocida"),
						FileUtils.getResourceBundle("etiqueta_el_puesto") + " " + post + " "
								+ FileUtils.getResourceBundle("etiqueta_se_apago_durante_uso"));
		...
		// In qualunque altro caso si procede con la modifica dello stato e
		// l'aggiornamento di view e seggi ausiliari
	...
	/**
	 * Funzione richiamata in risposta ad una richiesta del seggio ausiliario, dopo
	 * che questo legge una card dal lettore RFID.
	 * 
	 * @param intCard La la card letta dal seggio ausiliario.
	 * @param voter   L'eventuale votante che si è registrato presso il seggio
	 *                ausiliario.
	...
	synchronized Message readCardForSubStation(String intCard, Person voter) throws PEException {
		...
		// Dati i controlli sul Service, se voter non è null allora sto creando una
		// nuova associazione
		if (voter != null) {
			...
			try {
				...
				response.setElement("details",
						"Il voto è stato ricevuto dall'urna ma non è stato possibile informare la postazione");
			}
		}
	...
	void deactivateStation() {
		...
		if (Constants.verbose)
			printWarning("Terminale Disattivato",
					"L'urna è stata spenta, il seggio è stata riportata allo stato 'NON ATTIVO'");
	 ...
	 * Ogni funzione che richiede di essere in un particolare stato richiama questa
	 * per il controllo.
	 * 
	...
	/**
	 * Función que asocia un votante, el que se está registrando, a la insignia
	 * pasado y a un puesto elegido al azar entre las disponibles. Retirada cuando
	 * una insignia no asociada a otros votantes es leída por el lector RFID.
	 * 
	 * @param badge El valor leído por el lector RFID.
	 * @return Verdadero o falso dependiendo del resultado de la operación.
	 */

	/**
	 * Funzione che associa un votante, quello in fase di registrazione, al badge
	 * passato ed a una postazione scelta casualmente tra quelle disponibili.
	 * Richiamata quando un badge non associato ad altri votanti è letto dal reader
	 * RFID.
	 * 
	...
	private boolean createAssociation(String badge) {
		...
		// Il votante è stato assegnato a una postazione, si può registrare un nuovo
		// votante
		station.setNewVoter(null);

		// Si aggiorna la view e si informa lo staff con un messaggio
		String associationMsg = FileUtils.getResourceBundle("etiqueta_puesto_puntos") + " " + associatedPost;
		if (Constants.verbose)
			associationMsg += "\nBadge: " + badge + "\nVotante: " + voter.getLastName() + ", " + voter.getFirstName();
		printSuccess(FileUtils.getResourceBundle("etiqueta_asociacion_exitosa"), associationMsg);
		updateView();
	...
	/**
	 * Funzione richiamata quando dal reader RFID o da un seggio ausiliario arriva
	 * la richiesta di creazione di una associazione.
	 * 
	...
	private int createAssociationLogic(Person voter, String badge) throws PEException {
		// Riserviamo la postazione in modo che non possa essere assegnata a due votanti
		// contemporaneamente
		if (!station.reservePost()) {
			throw FLRException.FLR_05();
		}

		// Da questo momento in poi non si potranno fare nuove associazioni finchè non
		// avremo terminato con questa.
		...
		try {
			...
			// Contattiamo la postazione + Inviamo la richiesta di associazione e attendiamo
			// la risposta
			Message response;
			String errMsg = "Associazione fallita, impossibile contattare la postazione " + post + ".\n"
					+ "Controllare che sia la postazione " + post + " che questo seggio siano connessi alla rete.";
	...
	/**
	 * Funzione eseguita prima di una associazione per verificare la possibilità del
	 * votante.
	 * 
	...
	private void beforeAssociationControls(Person voter) throws PEException {
		...
		// Si verifica che il votante possa votare, e che gli siano stati assegnate
	...
	/**
	 * Funzione che verifica che il votante non sia già stato assegnato ad una delle
	 * postazioni.
	 * 
	...
	/**
	 * Funzione che invia il voto relativo al badge letto, all'urna, insieme ai dati
	 * sul votante.
	 * 
	...
	boolean sendVote(String badge, boolean updatePost) {
		...
		try {
			...
			printSuccess(FileUtils.getResourceBundle("etiqueta_voto_enviado_exito"),
					FileUtils.getResourceBundle("etiqueta_preguntas_rellenadas_puesto") + " " + id + " "
							+ FileUtils.getResourceBundle("etiqueta_enviados_urna"));
		} catch (PEException pee) {
			pee.printStackTrace();
			printWarning("Voto Inviato con Successo", "Le schede compilate nella postazione " + id
					+ " sono state inviate all'urna, " + "ma non è stato possibile notificarlo alla postazione.");
		}

		// Aggiorniamo la view e i seggi ausiliari e stampiamo a schermo un messaggio
		// informativo
		updateViewAndSubstation();
	...
	/**
	 * Funzione richiamata quando dal reader RFID o da un seggio ausiliario arriva
	 * la richiesta di invio voti all'urna.
	 * 
	 * @param badge Il badge usato come riferimento alla associazione.
	 */
	...
	/**
	 * Funzione che contatta una postazione al termine di un voto per farla tornare
	 * nello stato ATTIVA, rendendola di nuovo disponibile per ulteriori votazioni.
	 */
	private void notifyVoteConclusionToPost(int post) throws PEException {
		...
		String errMsg = failure + "\nAssicurarsi che sia la postazione " + post
				+ " che questo seggio siano connessi alla rete.";
		String ipRecipient = station.getPostIp(post).getHostAddress();
		try (Link link = createLink(station.getPostIp(post), station.getPostPort(), null, errMsg)) {
			if (link.isClosed())
				return;
	...
	/**
	 * Funzione che invia una richiesta a tutte le postazioni affinché queste
	 * inviino il loro stato.
	 */
	...
	/**
	 * Funzione che restituisce un thread per contattare e farsi restituire lo stato
	 * di una postazione. Si adoperano i thread in modo da velocizzare l'attivazione
	 * del seggio, che se no richiederebbe, un tempo lineare nel numero di
	 * postazioni.
	 * 
	...
	/**
	 * Funzione creata solo perchè a Marco seccava scrivere ogni volta 2 righe
	 * anzichè una sola.
	 */
	...
	/**
	 * Funzione che inoltra le informazioni sulle postazioni ad un seggio
	 * ausiliario.
	 * 
	...
	public synchronized String readCard(String card) {
		...
		// Usato unicamente per non lasciare il thread del reader RFID in attesa dopo lo
		// spegnimento
		if (card.equals(CardReader.exit)) {
			return null;
		}

		// Si verifica se il badge era già associato ad una postazione o se sta venendo
		// usato per crearne una nuova
		...
		if (post == -1) {
			...
			if (station.getNewVoter() == null) {
				printError(
						FileUtils.getResourceBundle("etiqueta_imposible_crear_asociacion") + " "
								+ FileUtils.getResourceBundle("etiqueta_selecciona_votante_antes_de_proceder"),
						FileUtils.getResourceBundle("etiqueta_asegurate_seleccionar_votante"));
			...
			if (docType == null) {
				printError(
						FileUtils.getResourceBundle("etiqueta_imposible_crear_asociacion") + " "
								+ FileUtils.getResourceBundle("etiqueta_falta_documento"),
						FileUtils.getResourceBundle("etiqueta_selecciona_tipo_documento"));
				return Protocol.associationNack;
			}
			...
			if (documentID.length() == 0 && !docType.equals(Person.DocumentType.CONOSCENZA_PERSONALE)) {
				printError(
						FileUtils.getResourceBundle("etiqueta_imposible_crear_asociacion") + " "
								+ FileUtils.getResourceBundle("etiqueta_falta_documento"),
						FileUtils.getResourceBundle("etiqueta_selecciona_conocimiento_personal"));
				return Protocol.associationNack;
			}
	...
	protected void invalidAuthentication() {
		...
		printError("Autenticazione Non Valida",
				"L'urna non riconosce questo seggio come valido, è necessario effettuare nuovamente l'autenticazione.");
		updateViewAndSubstation();
	}
	...
	public List<String> getBadgeCodeList() {
		return this.badgeCodeList;
	}

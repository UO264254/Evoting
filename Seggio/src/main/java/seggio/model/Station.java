...
public class Station extends AbstrModel {
	...
	/**
	 * Función llamada a la activación del asiento cuando la urna informa de las IP
	 * de los puestos y puestos auxiliares.
	 * 
	 * @param ipPosts       Direcciones IP de las ubicaciones.
	 * @param ipSubStations Direcciones IP de los puestos auxiliares.
	 */

	/**
	 * Funzione richiamata all'attivazione del seggio quando l'urna rende noti gli
	 * ip di postazioni e seggi ausiliari.
	 * 
	...
	public void init(InetAddress[] ipPosts, InetAddress[] ipSubStations) {
		...
		// Creamos una matriz de DummyPost para realizar un seguimiento de las
	...
	/**
	 * Función utilizada para sincronizar la asociación de los puestos. Se usa
	 * un monitor de modo que solo se utilice un asiento (principal o auxiliar)
	 * pueda crear a la vez una asociación.
	 * 
	 * @return Verdadero o falso dependiendo de si se ha reservado un puesto, por lo
	 *         que e el monitor está todavía en uso, o no, en cuyo caso el monitor
	 *         ha sido liberado.
	 */

	/**
	 * Funzione adoperata per sincronizzare l'associazione delle postazioni. In
	 * pratica adopera un monitor in modo che solo un seggio (principale o
	 * ausiliario) per volta possa creare una associazione.
	 * 
	 * @return Vero o falso a seconda se una postazione è stata riservata, per cui
	 *         il monitor è ancora in uso, o no, nel qual caso il il monitor è stato
	 *         rilasciato.
	 */
	public synchronized boolean reservePost() {
		// Esperamos que los que están trabajando para crear una asociación de términos
		...
		// Recuperiamo tutte le postazioni disponibili per l'associazione (tutte quelle
		// attive)
		...
		// indicando in questo modo l'impossibilità di creare altre associazioni per il
		// momento
	...
	/**
	 * Funzione da richiamare dopo aver terminato la creazione di una associazione,
	 * sia che questa sia riuscita o no, per liberare il monitor e permettere la
	 * creazione potenziale di altre associazioni.
	 */
	...
	/**
	 * Funzione che setta i valori memorizzati di una postazione quando si effettua
	 * un reset.
	 * 
	 * @param index    La postazione per cui si è effettuato il reset.
	 * @param shutDown Se true questo flag indica che la postazione è andata
	 *                 offline, non semplicemente resettata.
	 */
	...
	public StateStation getState() {
		return state;
	}

	public void setState(StateStation state) {
		this.state = state;
	}

	public InetAddress[] getSubStationIps() {
		return ipSubStations;
	}

	public int getSubStationPort() {
		return subStationPort;
	}
	...
	public void setPostState(StatePost state, DummyPost post) {
		...
		List<StatePost> votingStates = List.of(StatePost.ASSOCIATA, StatePost.IN_USO, StatePost.VOTO_INVIATO,
	...
	public InetAddress getPostIp(int index) {
		return posts[index].getIp();
	}

	public Person getPostVoter(int index) {
		return posts[index].getVoter();
	}

	public String getPostBadge(int index) {
		return posts[index].getBadge();
	}

	public void setNewVoter(Person voter) {
		newVoter = voter;
	}

	public Person getNewVoter() {
		return newVoter;
	}

	public void setDocumentType(String type) {
		this.newVoter.setDocumentType(type);
	}

	public Person.DocumentType getDocumentType() {
		return newVoter != null ? newVoter.getDocumentType() : null;
	}

	public void setDocumentID(String id) {
		this.newVoter.setDocumentID(id);
	}

	public String getDocumentID() {
		return newVoter != null ? newVoter.getDocumentID() : null;
	}

	public void setEmptyBallots(EmptyBallot[] ballots) {
